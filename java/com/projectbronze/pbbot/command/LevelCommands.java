...
public class LevelCommands implements ICommandList {
	...
	public List<Command> getCommands() {
		return Arrays.asList(new Command[] { createCommand("админ+", "адм+", "Добавляет администратора", "<Имя|ID> <Уровень>", (msg, args, guild) -> {
			if (args.length != 2) {
			...
			if (LevelUtils.getLevel(usr) >= LevelUtils.getLevel(msg.getAuthor())) {
				reply(msg, "Не дам");
				return;
			}
