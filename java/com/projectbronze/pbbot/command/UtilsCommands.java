...
public class UtilsCommands implements ICommandList {
	...
	public List<Command> getCommands() {
		return Arrays.asList(new Command[] { createCommand("умри", "умр", "Убивает бота", "", (msg, args, guild) -> {
			if (args.length == 0) {
				Core.exit(0, null);
			} else {
		...
		}, 70), createCommand("команды", "хэлп", "Показывает команды, если указана команда показывает информацию о ней. Хотя вы ведь и так это знаете?", "[команда|-синтаксис]", (msg, args, guild) -> {
			if (args.length != 0) {
			...
			if (args.length == 0) {
				reply(msg, "Вы не сказали мне что говорить.");
				return;
			}
			String rep = "";
			for (String s : args) {
			...
			reply(msg, "Ну неееет.");
		}, 100), createCommand("рандом", "ранд", "Выбирает случайный из указаных аргументов", "<арг1> <арг2> [argN]", (msg, args, guild) -> {
			reply(msg, args[Core.rand.nextInt(args.length)]);
		}) });
	}
}
